# Contributing to Lighthouse Box

Thank you for your interest in contributing to Lighthouse Box! This document provides guidelines for contributing to the project.
...
### Scopes for Lighthouse Box

| Scope        | Description                   |
|--------------|-------------------------------|
| `action`     | GitHub Action configuration   |
| `lighthouse` | Lighthouse audit logic        |
...
feat(lighthouse): add PWA category support
feat(gist): add emoji badges option

# Bug fixes
fix(lighthouse): resolve Chrome launch issue
...
```bash
git clone https://github.com/abordage/lighthouse-box.git
cd lighthouse-box
npm ci
```
...
```bash
git commit -m "feat(lighthouse): add accessibility score threshold"
```
...
Thank you for contributing to Lighthouse Box!
